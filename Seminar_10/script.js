...
// const dayWeekNumber = [1, 2, 3, 4, 5, 6, 7];
// const week = {};
// for (let i = 0; i < 7; i++) {
//   week[dayWeekNumber[i]] = dayWeek[i];
// }
// console.log(week);
...
const riddle = {
  ...
  hints: ["это зеленое", "это с иголками", "наряжаем на новый год"],
  askQuestion() {
    let countLoose = 0;
    while (countLoose <= this.hints.length) {
      const userAnswer = prompt(this.question).toLocaleLowerCase();
      if (this.answer.includes(userAnswer)) {
        alert("Поздравляю");
        return;
      } else {
        if (countLoose < this.hints.length) {
          alert(this.hints[countLoose]);
          countLoose++;
        } else {
          alert("Не верно!");
          return;
